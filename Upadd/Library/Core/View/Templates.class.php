...
class Templates{
	 ...
	 * @param string $_File
	 */
	public function setFile($file='', $cache=0){
		if ($cache) {
    ...
    */
	...
	public function setInclude(){
		$preFile = '/<\!--\${(.*)\}\-->/';
		if (preg_match($preFile, $this->_fileVar, $file)){
			//$setFile = HTML_PAHT.$file[1];
			if (!file_exists($file[1]) || empty($file[1])) {
				exit('您引入的文件有错误!');
			}
			// include \"$1"; 
			$this->_fileVar =  preg_replace($preFile, "<?php echo '$1'; ?>", $file[1]);
		}
	}
