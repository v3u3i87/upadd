...
class Tool{
	...
	//弹窗跳转
	static public function test($_info, $_url) {
		LoadUed('css/base.css',array('plug'=>'css','path'=>'admin'));
		LoadUed('css/style.css',array('plug'=>'css','path'=>'admin'));
		LoadUed('js/jquery.js',array('plug'=>'js','path'=>'admin'));
		LoadUed('js/base.js',array('plug'=>'js','path'=>'admin'));
		echo "<script>$(function(){ M._alert('{$_info}');history.back();})</script>";
		exit();
	}
	...
	public static function ajaxload($url=''){
		//载入AJAX.JS异步加载脚本
		LoadUed('ajaxfileupload.js',array('plug'=>'js','path'=>'js'));
		$html='<script>
	...
	public static function bdedit($edit,$js,$diy=0){
		if($js=='js'){
			LoadUed('ueditor.all.js',array('plug'=>'js','path'=>'ueditor'));
			LoadUed('ueditor.config.js',array('plug'=>'js','path'=>'ueditor'));
		}
		if($diy){
			$html="
					<script>
