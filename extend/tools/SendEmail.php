...
$config = [
    //【必须】，发送的smtp服务器
    'sendServer' => 'smtp.exmail.com',
    //【必须】，开启smtp的邮箱
    'sendServerEmail' => 'server@exmail.com',
    //【必须】，smtp邮箱的密码
    'sendServerEmailPassword' => 'exmail',
    //【必须】，发送邮件的端口号
    'sendServerPort' => 25,
    //【必须】，发送邮件的邮箱，可能更改，一般为开启smtp的邮箱本身
    'fromEmail' => 'server@exmail.com',
    // 【非必须，可为空】，显示发送方的名称
    'fromEmailShowName' => 'exmail',
];

$semdemail = new SendEmail($config);
$semdemail->to('office@zmq.cc');
$semdemail->body('测试',"哈哈");
if (true === $semdemail->send())
{
    echo "ok\r\n";
} else {
    echo "on\r\n";
...
class SendEmail
{
    ...
    protected function getCommand() {
        ...
        if(isset($this->_attachment)){
            foreach ($this->_attachment as $attachment){
                $header .= "\r\n--" . $separator . "\r\n";
                $header .= "Content-Type: " . $this->getMIMEType($attachment) . '; name="' . basename($attachment) . '"' . "\r\n";
                $header .= "Content-Transfer-Encoding: base64\r\n";
                $header .= 'Content-Disposition: attachment; filename="' . basename($attachment) . '"' . "\r\n";
                $header .= "\r\n";
                $header .= $this->readFile($attachment);
                $header .= "\r\n--" . $separator . "\r\n";
            }
        }
        $header .= "\r\n.\r\n";
     ...
     * @return mixed
     */
    protected function readFile($attachment) {
        if( isset($attachment) ) {
     ...
     * @return mixed
     */
    protected function getMIMEType($attachment) {
        if( isset($attachment) ) {
            $mime = mime_content_type($attachment);
            if ($mime){
                if(! preg_match("/gif|jpg|png|jpeg|pdf|doc|docx|xlsx|xls|ppt|pptx/", $mime)){
